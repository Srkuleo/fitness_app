...
import { type RadioButtonProps, type EditingProps } from "../../types/types";
...
const RadioButtonsContent = ({
  ...
  toggleAdding,
  closeDropDown
}: EditingProps & Record<"closeDropDown", () => void>) => {
  const [workouts, setWorkouts] = useState<RadioButtonProps[]>([]);
  const [newWorkout, setNewWorkout] = useState<RadioButtonProps>(initialWorkout);
    ...
    <>
      {isAdding && (
        <div
          ...
          className='fixed top-0 z-20 h-screen w-screen cursor-default bg-black/80'
        >
          <div className='center absolute top-[50%] left-[50%] flex flex-col gap-10 rounded-lg bg-slate-light50 p-20'>
            <p className='text-lg font-semibold text-slate-main600'>
              Please fill all fields before pressing {`"Add"`} button.
            </p>
            <form
      ...
      <div className='relative z-0 mx-auto flex flex-col items-center'>
        ...
        {workouts.length === 0 ? (
          ...
          <>
            <RadioButtons workouts={workouts} toggleIsSelected={toggleIsSelected} />
            <StartButton />
          </>
        )}
      </div>
